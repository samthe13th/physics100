...
function getArrowByAngle(a) {
    ...
    function render() {
        ...
        var aaStr4 = "";
        if (cArrow != null) {
            game.debug.text(arrowHere() + " cArrow = " + cArrow.fType + " (" + cArrow.axis + ")" + " [" + cArrow.mag + "]", 0, 390);
        }
        if (returnArrow1 != null) {
            game.debug.text("R1(rel)= " + returnArrow1.fType + " " + returnArrow1.axis + " " + returnArrow1.mag, 0, 320);
        }
        if (returnArrow2 != null) {
            game.debug.text("R2(abs)= " + returnArrow2.fType + " " + returnArrow2.axis + " " + returnArrow2.mag, 0, 350);
        }
        if (arrowWithMagFound(returnArrow1, returnArrow2)) {
            var rArrow = returnArrowWithMag(returnArrow1, returnArrow2);
            game.debug.text("Arrow with mag found! (" + rArrow.axis + ")", 0, 370);
        }
